...
# only valid indices, with -1 being the index for the first element.
print(nums[-1])
print(nums[-5])

# The Python lenn function tells us the number of elements in a list.
print(len(nums))
...
#A handy way to step through the lements in a list is by using a loop.
...
# These lines of code add multiple elemets to the lsit using a loop.
for i in range(1,5):
    names.append(i)
...
# The remove function only remove one element at a time
...
# To remove an element from a spesific position of a list, the index of the element
